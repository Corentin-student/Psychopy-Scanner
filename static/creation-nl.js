...
function submit(name) {
    ...
    if (isOverlap) {
        alert("Er is op dit moment al een stimulus bezig of er is een overlap zonder overeenkomstige duur.");
        return;
    }
...
function checkingHoles(){
    ...
    if (rows.length > 0) {
        ...
        if (firstRowTiming !== 0) {
            ...
            discontinuities.push(`Discontinuïteit gedetecteerd tussen 0s en ${firstRowTiming}s`);
        }
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (lastEnd < rowTiming && i > 0) {
            if (!timingMap.hasOwnProperty(lastEnd)) {
                ...
                discontinuities.push(`Discontinuïteit gedetecteerd tussen ${lastEnd}s en ${rowTiming}s`);
            }
        }
    ...
    if (discontinuities.length > 0) {
        ...
        console.log("Geen discontinuïteit gedetecteerd.");
    }
    return timingMap;
}
function handleFileUpload(event) {
    ...
    if (file) {
        ...
        .catch(error => console.error('Fout bij uploaden bestand:', error));
    }
}
