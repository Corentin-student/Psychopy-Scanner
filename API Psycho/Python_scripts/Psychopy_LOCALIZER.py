...
class Localizer(Parente):
    ...
    def lancement(self):
        ...
        nb_groups = len(self.real_groups)
        if nb_groups == 0:
            raise ValueError("Aucun groupe trouvé dans le fichier d'input, impossible de lancer le paradigme.")
        ...
        for x in range(self.number_of_blocks):
            ...
            if self.random:
                self.show_block(random.randint(0, nb_groups - 1), self.number_per_block)
            else:
                y = x % nb_groups
    ...
    def show_block(self, index, number_per_block):
        ...
        # Vérifier qu'il y a assez de stimuli disponibles
        if len(self.copy_real_groups[index]) == 0:
            raise ValueError(f"Le groupe {index} ne contient aucun stimulus.")
        ...
        while len(toshow) < number_per_block:
            if self.real_groups[index] != []:
                ...
                # Recharger depuis la copie et réinitialiser
                self.real_groups[index] = copy.deepcopy(self.copy_real_groups[index])
                if self.random:
                    stimuli = random.choice(self.real_groups[index])
                else:
                    stimuli = self.real_groups[index][0]
